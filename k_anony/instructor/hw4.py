...
def link_attack(df, attack_df, qsi):
    ...
    output:
    """
    return pd.merge(df, attack_df, on = qsi)
...
def main():
    ...
    """
    column_names = ['fnlwgt', 'education', 'relationship', 'capital_gain', 'capital_loss', 'hours_per_week']
    suppressed_path = suppression(df, column_names)
